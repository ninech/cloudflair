...
describe Cloudflair::EmailObfuscation do
  ...
  let(:response_json) { File.read('spec/cloudflair/fixtures/zone/email_obfuscation.json') }
  let(:url) { "/client/v4/zones/#{zone_identifier}/settings/email_obfuscation" }
  let(:subject) { Cloudflair.zone(zone_identifier).settings.email_obfuscation }
  let(:value) { 'on' }
  let(:new_value) { 'off' }
